...
void main() {
    ...
    float is_frag_transparent = 1.0 - smoothstep(0.4, 1.0, v_FragPos.x * v_FragPos.x + v_FragPos.z * v_FragPos.z);
    // float is_frag_transparent = 1.0;
    
    gl_FragColor = vec4(v_Color * lightColor * (u_Material.ambient * ambientIntensity + diffuse * u_Material.diffuse + specular * u_Material.specular), 1.0 * is_frag_transparent);
}
