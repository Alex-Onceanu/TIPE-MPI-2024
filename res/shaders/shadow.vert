...
void main() {
    ...
    if(obj_center.y < y_max)
    {
        ...
        float x_scale = length(obj_center.xz - normalize(obj_center_to_light.xz) - new_center.xz);
        x_scale = max(x_scale, 1.0);
        ...
        v_Zscale = 1.4;
        vec4 frag_world_pos = vec4(x_scale * a_Position.x, a_Position.y, v_Zscale * a_Position.z, 1.0) * shadow_transform * (u_Rotation * u_Translation);
