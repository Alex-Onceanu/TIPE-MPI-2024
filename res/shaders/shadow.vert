...
void main() {
    ...
    if(obj_center.y < y_max)
    {
        ...
        v_Xscale = length(obj_center.xz - normalize(obj_center_to_light.xz) - new_center.xz) / u_Radius;
        ...
        vec4 frag_world_pos = vec4(v_Xscale * u_Radius * a_Position.x, a_Position.y, u_Radius * a_Position.z, 1.0) * shadow_transform * (u_Translation);
    ...
    {
        vec4 frag_world_pos = vec4(u_Radius * a_Position.x, a_Position.y, u_Radius * a_Position.z, 1.0) * (u_Translation);
        v_Xscale = 1.0;
